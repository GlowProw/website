              ...
              <v-list-item
                ...
                <template v-slot:prepend>
                  <v-checkbox
                      ...
                      @update:model-value="onToggleLayer(category.value)"
                      hide-details
                      class="mr-2"
...
import {createPoint, getCollectionPoints, getCollections, getNearbyPoints} from '@/assets/sripts/map_service';
...
onMounted(async () => {
  ...
  const vectorLayer = new VectorLayer({
    ...
    style: (feature) => {
      ...
      if (featureCategory === 'shareLocation') {
        ...
        return isPersonalVisible ? onCreatePersonalMarkerStyle(feature) : null;
      }
  ...
  map.on('dblclick', (event) => {
    ...
    newMarkerData.value = {
      id: '',
      collectionId: selectedCollectionUuid.value || (userCollections.value[0]?.id || ''),
      title: '',
      description: '',
      longitude: coordinate[0],
      latitude: coordinate[1],
...
const onResetNewMarkerData = (): void => {
  newMarkerData.value = {
    id: '',
    collectionId: selectedCollectionUuid.value || '',
    title: '',
    description: '',
    longitude: 0,
    latitude: 0,
 ...
 * 创建个人标记样式
 */
const onCreatePersonalMarkerStyle = (feature: OLFeature<Geometry>): Style => {
  return new Style({
    image: new Icon({
      src: getPersonalMarkerIcon(),
      scale: 0.1,
 ...
 * @param category
 */
const onToggleLayer = (category: string) => {
  if (!vectorLayerRef.value) return;
...
const onToggleAllLayers = () => {
  ...
  vectorLayerRef.value.setStyle((feature) => {
    ...
    if (featureCategory == 'shareLocation') {
      return newVisibility ? onCreatePersonalMarkerStyle(feature) : null;
    }
...
const getLocationDisplayName = (data: any): string => {
  ...
  return asString([
    location.id,
    `snb.mapLocations.${location.category}${onCapitalizeFirstLetter(location.id)}.name`,
    `snb.mapLocations.${onCapitalizeFirstLetter(location.id)}.name`,
 ...
 * @param str
 */
const onCapitalizeFirstLetter = (str: string): string => {
  if (!str) return str;
